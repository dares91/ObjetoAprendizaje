	...
	*/
	...
	function gameFinished()
	{
		...
		if(correctSquares == ((cols * rows) -1)){
			document.getElementById('messageDiv').innerHTML = '<h2>Fantastico! lo hicistes !!</h2>';
			gameInProgress = false;
			revealImage();
			
		}else{
			document.getElementById('messageDiv').innerHTML = 'Actualmente, tienes ' + correctSquares + ' de ' + ((cols * rows) -1) + ' piezas colocadas correctamente';
		}
	...
	function moveImage(e,fromShuffleFunction)
	{
		...
		if(!gameInProgress && !fromShuffleFunction){
			alert('No has desordenado los bloques');
			return;
		}
